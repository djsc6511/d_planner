...
/**
 * Created by Raquib-ul-Alam Kanak on 7/21/2014. Website: http://alamkanak.me
 */
public class WeekView extends View {
     ...
     * An EventRect is actually the rectangle that is drawn on the calendar for
     * a given event. There may be more than one rectangle for a single event
     * (an event that expands more than one day). In that case two instances of
     * the EventRect will be used for a single event. The given event will be
     * stored in "originalEvent". But the event that corresponds to rectangle
     * the rectangle instance will be stored in "event".
     */
    private class EventRect {
         ...
         * rectangle that is drawn on the calendar for a given event. There may
         * be more than one rectangle for a single event (an event that expands
         * more than one day). In that case two instances of the EventRect will
         * be used for a single event. The given event will be stored in
         * "originalEvent". But the event that corresponds to rectangle the
    ...
    private void getMoreEvents(Calendar day) {
        ...
        if (mFetchedMonths[1] < 1
            ...
            if (!containsValue(lastFetchedMonth, day.get(Calendar.MONTH) + 1)
                ...
                Log.d("", "currentMonthData" + events.size());
                sortEvents(events);
                for (WeekViewEvent event : events) {
                    cacheEvent(event);
                }
        ...
        if (mFetchedMonths[2] < 1 || mFetchedMonths[2] != nextMonth
            ...
            if (!containsValue(lastFetchedMonth, nextMonth) && !isInEditMode()) {
                ...
                Log.d("", "nextMonthData"+events.size());
                sortEvents(events);
                for (WeekViewEvent event : events) {
                    cacheEvent(event);
                }
     ...
     *
     * @param hour
     *            The hour to scroll to in 24-hour format. Supported values are
     *            0-24.
     */
    public void goToHour(double hour) {
        if (hour < 0)
            throw new IllegalArgumentException(
                    "Cannot scroll to an hour of negative value.");
        else if (hour > 24)
            throw new IllegalArgumentException(
                    "Cannot scroll to an hour of value greater than 24.");
        else if (hour * mHourHeight > mHourHeight * 24 - getHeight()
                + mHeaderTextHeight + mHeaderRowPadding * 2
                + mHeaderMarginBottom)
            throw new IllegalArgumentException(
                    "Cannot scroll to an hour which will result the calendar to go off the screen.");
